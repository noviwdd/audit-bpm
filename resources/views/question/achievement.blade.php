            ...
            @foreach ($questions as $question)
                <div class="p-6 m-2 my-4 bg-white shadow rounded-lg">
                    ...
                    @if (isset($question['type']) && $question['type'] === 'input')
                        <div>
                            <p class="mb-2.5 whitespace-pre-wrap">{{ $question['question']['main'] }}</p>
                            @foreach ($question['question'] as $key => $inputQuestion)
                                            ...
                                            @foreach ($inputQuestion as $subkey => $subquestion)
                                                @if ($subkey !== 'sub_main')
                                                    <div class="mb-3">
                                                        ...
                                                        <input type="text" id="{{ $question['code'] . $subkey }}" name="answers[{{ $question['code'] }}][{{ $subkey }}]" value="{{ old('answers.' . $question['code'] . '.' . $subkey, $answers[$question['code'] . '-' . $subkey]->achieve_answer ?? '') }}" class="block w-full p-2 text-gray-900 border border-gray-300 rounded-lg text-xs focus:ring-caribbean focus:border-caribbean">
                                                    </div>
                                                @endif
                                            @endforeach
                                        </div>
                                ...
                                @elseif ($key !== 'main')
                                    <div class="mb-4">
                                        ...
                                        <input type="text" id="{{ $question['code'] . $key }}" name="answers[{{ $question['code'] }}][{{ $key }}]" value="{{ old('answers.' . $question['code'] . '.' . $key, $answers[$question['code'] . '-' . $key]->achieve_answer ?? '') }}" class="block w-full text-xs text-gray-900 border border-gray-300 rounded-lg focus:ring-caribbean focus:border-caribbean">
                                    </div>
                                @endif
                            @endforeach
                        </div>
                    @else
                        <div>
                            <li>
                                <p class="mb-6 leading-relaxed whitespace-pre-wrap">{{ $question['question'] }}</p>
                                <ul>
                                    @foreach ($question['choices'] as $key => $choice)
                                        <li class="mb-2">
                                            <input type="radio" id="{{ $question['code'] . $key }}" name="answers[{{ $question['code'] }}]" value="{{ $key }}" class="hidden peer" {{ old('answers.' . $question['code'], $answers[$question['code']]->achieve_answer ?? '') == $key ? 'checked' : '' }} />
                                            <label for="{{ $question['code'] . $key }}"
                                                ...
                                                @if (old('answers.' . $question['code'], $target[$question['code']]->target_answer ?? '') == $key)
