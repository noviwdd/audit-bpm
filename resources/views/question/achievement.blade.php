...
@section('content')
    ...
    <form method="POST" action="{{ route('achievement.save') }}">
        @csrf
        <ul>
            <div class="flex justify-between px-6 py-3 m-2 my-4 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                <p class="font-bold">{{ $currentCriteria }}</p>
                <p class="font-bold">{{ $answeredCount }}/{{ $questionCounts[$currentCriteria] }} Pertanyaan</p>
            </div>
            @foreach ($questions as $question)
                <div class="p-6 m-2 my-4 bg-white shadow rounded-lg dark:bg-gray-800 dark:border-gray-700">
